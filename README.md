...
- **LedgerSMB**: `LEDGERSMB_BASE_URL` enables the provider adapter boundary; optional `LEDGERSMB_DATABASE`, `LEDGERSMB_USERNAME`/`LEDGERSMB_PASSWORD`, or `LEDGERSMB_API_KEY` configure company selection and auth. The current scaffolding normalizes accounts and journal entries from `/api/accounts` and `/api/journal-entries` without changing the financial UI.
