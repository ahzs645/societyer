...
- **Wave**: set `WAVE_ACCESS_TOKEN` and `WAVE_BUSINESS_ID` to switch the accounting sync from demo data to live GraphQL fetches. `WAVE_CLIENT_ID` and `WAVE_CLIENT_SECRET` are reserved for OAuth setup and are reported only as present/missing in diagnostics; `WAVE_GRAPHQL_ENDPOINT` is an optional override for the default Wave GraphQL endpoint. The Financials page includes a Wave health check that reports only present/missing env status plus redacted provider diagnostics.
