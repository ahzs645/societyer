// Workflow recipe catalog + helpers extracted from workflows.ts: recipe/node
// definitions and sample data, PDF inspection, template/context rendering, and the
// external-workflow execution helpers. The Convex query/mutation/action/internal*
// registrations stay in workflows.ts so all api/internal.workflows.* paths are unchanged.
...
function computeEffectiveNodePreview(
