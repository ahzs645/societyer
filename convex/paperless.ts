...
function transposedBudgetSnapshot(doc: any) {
  ...
  return {
    ...
    sourceDate,
    currency: "CAD",
    totalIncomeCents: moneyNear(text, /\b(total income|income|revenue)\b/i),
    totalExpenseCents: moneyNear(text, /\b(total expenses?|expenses?|expenditures?)\b/i),
    netCents: moneyNear(text, /\b(net|surplus|deficit)\b/i),
    endingBalanceCents: moneyNear(text, /\b(ending balance|closing balance|cash balance)\b/i),
    status: "NeedsReview",
    notes: "Budget snapshot candidate. OCR amounts need reviewer verification before becoming official budget data.",
    confidence: "Review",
    sensitivity: "restricted",
...
function transposedTransactionCandidate(doc: any) {
  ...
  return {
    ...
    description: title,
    amountCents: moneyNear(text, /\b(amount|total|paid|deposit|withdrawal|balance due)\b/i),
    ...
    sensitivity: "restricted",
    confidence: "Review",
    notes: "Restricted transaction candidate. Keep raw banking/payroll/tax OCR out of public views; reconcile only after finance review.",
    sourceDate: inferredPaperlessDate(doc),
    sourceExternalIds: [`paperless:${doc.id}`],
  };
}
