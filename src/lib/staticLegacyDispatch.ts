...
// These writes do not follow the module-to-table convention and must not receive
// fake success from generic CRUD. See docs/local-portable-migration-backlog.md.
const STATIC_GENERIC_CRUD_DENYLIST: ReadonlySet<string> = new Set([
  "apiPlatform:createToken",
  "apiPlatform:upsertWebhookSubscription",
  "paperless:createBylawsHistoryImportSession",
  "paperless:createDiscoveryImportSession",
  "paperless:createMeetingMinutesImportSession",
  "paperless:createTransposedImportSession",
  "secrets:update",
]);
...
export function mutableQueryResult(name: string, args: StaticArgs, store?: StaticDemoDexieStore | null) {
 ...
 */
export function portableSyncStub(name: string, args: StaticArgs, store?: StaticDemoDexieStore | null) {
  const result = store?.queryResult(name, args) ?? queryResult(name, args, store);
...
export function mutationResult(name: string, args: StaticArgs, store?: StaticDemoDexieStore | null): any {
  ...
  if (STATIC_GENERIC_CRUD_DENYLIST.has(name)) return reportStaticWriteGap(name);
