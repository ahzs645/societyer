/**
 * PORTABLE FUNCTIONS: the demo-society seeder (run / reset).
 *
 * Seeds the demo society "Riverside Community Society" and its full demo
 * dataset, or wipes everything. Reads/writes exclusively through the portable
 * `ctx.db` contract, so each handler runs unchanged on hosted Convex, the local
 * Dexie runtime, and the convex-test oracle.
 *
 * The maintenance-token gate (`assertMaintenanceToken`, which reads process.env)
 * stays on the Convex side (convex/seed.ts) and runs before the portable body.
 */
...
export async function runPortable(ctx: PortableMutationCtx): Promise<{ societyId: string }> {
  await wipe(ctx);
  ...
  await ctx.db.insert("insurancePolicies", {
    ...
    policySeriesKey: "dno|intact-public-entities|management-liability",
    ...
    coverageItems: [
      { label: "Directors and officers liability", coverageType: "D&O liability", coveredClass: "directors_officers", limitCents: 200000000 },
    ],
    policyDefinitions: [
    ...
    declinedCoverages: [],
    claimsMadeTerms: {
      reportingDeadline: "2026-12-31",
      defenseCostsInsideLimit: true,
      retentionCents: 100000,
      territory: "Worldwide",
      sourceExternalIds: ["demo:insurance-dno-2026"],
      notes: "Demo terms only; verify actual policy wording.",
    },
    annualReviews: [
      {
        reviewDate: "2026-01-15",
        boardMeetingDate: "2026-01-15",
        reviewer: "Treasurer",
        outcome: "Renewed with D&O terms to verify",
  ...
  await ctx.db.insert("insurancePolicies", {
    ...
    policySeriesKey: "cgl|co-operators|cgl-17742",
    policyTermLabel: "2026",
    versionType: "event-policy",
    coverageCents: 500000000,
    premiumCents: 312000,
    coverageItems: [
      { label: "Commercial general liability", coverageType: "CGL", limitCents: 500000000 },
      { label: "Tenants legal liability", coverageType: "Premises / room coverage", limitCents: 50000000 },
    ],
    coveredLocations: [
      { label: "Demo facility room", room: "Needs review", coverageCents: 50000000, sourceExternalIds: ["demo:insurance-cgl-2026"] },
    ],
    declinedCoverages: [],
    certificatesOfInsurance: [
      {
        holderName: "Demo venue",
        additionalInsuredLegalName: "Demo venue and its governors, employees, and agents",
        eventName: "Spring community event",
        eventDate: "2026-05-15",
        requiredLimitCents: 500000000,
        status: "Needs review",
    ...
    insuranceRequirements: [
      {
        context: "Facility or room booking",
        requirementType: "event_or_facility",
        coverageSource: "Society policy",
        cglLimitRequiredCents: 500000000,
        cglLimitConfirmedCents: 500000000,
        additionalInsuredRequired: true,
        coiStatus: "Needs review",
        tenantLegalLiabilityLimitCents: 50000000,
        riskTriggers: ["non_student_attendees", "fees_or_donations"],
        sourceExternalIds: ["demo:insurance-cgl-2026"],
      },
    ],
    complianceChecks: [
...
export async function resetPortable(ctx: PortableMutationCtx): Promise<{ ok: boolean }> {
  await wipe(ctx);
  return { ok: true };
}

async function wipe(ctx: PortableMutationCtx) {
  const tables = [
    "activity",
    "tasks",
    "goals",
