/**
 * PORTABLE FUNCTIONS: the library domain (overview).
 *
 * Reads the `documents` and `meetingMaterials` tables over `ctx.db`, joins them
 * to meetings, and derives the reference-library + meeting-packet view. Runs
  ...
  const meetingPackets = meetingIds
    .map((meetingId) => {
      ...
      return {
        ...
        requiredCount: packetMaterials.filter((row: any) => row.requiredForMeeting).length,
      };
    })
