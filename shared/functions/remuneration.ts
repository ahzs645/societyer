/**
 * PORTABLE FUNCTIONS: the remuneration domain
 * (disclosureForYear / applyToFinancials).
 *
 * Reads the `employees` table and writes the `financials` table over `ctx.db`.
 * Each handler runs unchanged on hosted Convex, the local Dexie runtime, and the
 * convex-test oracle.
 */

import type { PortableMutationCtx, PortableQueryCtx } from "../portable/ctx";
import { requireOwnedRow, requireSocietyMembership } from "./access";

/**
 * Build the ≥ $75k remuneration disclosure note (s.36) from employee records
 * for a given fiscal year. Returns rows by position + totals.
 */
export async function disclosureForYearPortable(
  ctx: PortableQueryCtx,
  { societyId, fiscalYear }: { societyId: string; fiscalYear: string },
) {
  await requireSocietyMembership(ctx, societyId);
  const employees = await ctx.db
    .query("employees")
    .withIndex("by_society", (q) => q.eq("societyId", societyId))
    .collect();
  const earners = employees
    .map((e: Record<string, any>) => {
      const annual =
        e.annualSalaryCents ??
        (e.hourlyWageCents ? e.hourlyWageCents * 40 * 52 : 0);
      return { role: e.role, amountCents: annual, person: `${e.firstName} ${e.lastName}` };
    })
    .filter((x) => x.amountCents >= 7_500_000)
    .sort((a, b) => b.amountCents - a.amountCents)
    .slice(0, 10);
  const totalCents = earners.reduce((s, r) => s + r.amountCents, 0);
  return {
    fiscalYear,
    count: earners.length,
    totalCents,
    byPosition: earners.map((e) => ({ role: e.role, amountCents: e.amountCents })),
  };
}

/** Apply the computed disclosure back onto a financials row. */
export async function applyToFinancialsPortable(
  ctx: PortableMutationCtx,
  { financialsId, disclosures }: { financialsId: string; disclosures: { role: string; amountCents: number }[] },
): Promise<void> {
  await requireOwnedRow(ctx, "financials", financialsId);
  await ctx.db.patch(financialsId, { remunerationDisclosures: disclosures });
}
