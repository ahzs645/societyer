/**
 * PORTABLE FUNCTIONS: the AI provider settings domain
 * (getEffective / upsert / setStatus).
 *
 * Read/write CRUD over the `aiProviderSettings` table via `ctx.db`, role-gated
 * through `requireRolePortable`. Each handler runs unchanged on hosted Convex,
 * the local Dexie runtime, and the convex-test oracle. The model-catalog cache
 * internal functions (`_getModelCatalogCache` / `_upsertModelCatalogCache`) and
 * anything that performs provider validation/fetch stay in convex/aiSettings.ts.
 */
...
export async function getEffectivePortable(
