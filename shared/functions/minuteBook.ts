/**
 * PORTABLE FUNCTIONS: the minute-book domain — the governance binder spine
 * (manual minute-book items) plus the cross-domain `overview` that stitches
 * meetings, minutes, filings, policies, evidence, and more into record bundles
 * and compliance checks.
 *
 * Reads/writes the `minuteBookItems` table and reads the related governance
...
function collectBySociety(ctx: PortableQueryCtx, table: string, societyId: any, limit: number) {
  return ctx.db
    .query(table)
...
async function collectBinderDocumentPreviews(ctx: PortableQueryCtx, societyId: any) {
