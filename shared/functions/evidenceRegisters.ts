/**
 * PORTABLE FUNCTIONS: the evidence-registers domain — governance/finance review
 * ledgers (board roles, signing authorities, attendance, motion evidence,
 * budgets, statement imports, transaction candidates, source evidence, archive
 * accessions) plus the curated Paperless review workflows.
 *
 * Reads/writes those register tables over `ctx.db`. Each handler runs unchanged
...
export async function updateReviewPortable(
...
export async function promoteBoardRoleToDirectorPortable(
  ctx: PortableMutationCtx,
  args: {
    assignmentId: string;
    position?: string;
...
export async function finishSafePaperlessReviewPortable(
  ctx: PortableMutationCtx,
  { societyId }: { societyId: string },
) {
  const completedAt = todayDate();
